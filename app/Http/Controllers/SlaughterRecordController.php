...
class SlaughterRecordController extends Controller
{
    ...
    private function rules(?int $ignoreId = null): array
    {
        return [
            ...
            'custom_adjustments.*.title' => ['required_with:custom_adjustments', 'string', 'max:255'],
            'custom_adjustments.*.amount' => ['required_with:custom_adjustments', 'numeric'],
            'rejected_piece_ids' => ['nullable', 'array'],
            'rejected_piece_ids.*' => ['string', 'max:255'],
            're_weight_entries' => ['nullable', 'array'],
            're_weight_entries.*.tag_id' => ['required_with:re_weight_entries', 'string', 'max:255'],
            're_weight_entries.*.original_weight' => ['required_with:re_weight_entries', 'numeric', 'min:0'],
            're_weight_entries.*.new_weight' => ['required_with:re_weight_entries', 'numeric', 'min:0'],
            'chiller_transfers' => ['nullable', 'array'],
            'chiller_transfers.*.chiller_name' => ['required_with:chiller_transfers', 'string', 'max:255'],
            'chiller_transfers.*.chiller_in_time' => ['required_with:chiller_transfers', 'string', 'max:255'],
            'chiller_transfers.*.plan_duration_hours' => ['required_with:chiller_transfers', 'numeric', 'min:0'],
            'blast_freezer_transfers' => ['nullable', 'array'],
            'blast_freezer_transfers.*.blast_freezer_name' => ['required_with:blast_freezer_transfers', 'string', 'max:255'],
            'blast_freezer_transfers.*.blast_freezer_in_time' => ['required_with:blast_freezer_transfers', 'string', 'max:255'],
            'blast_freezer_transfers.*.plan_duration_hours' => ['required_with:blast_freezer_transfers', 'numeric', 'min:0'],
